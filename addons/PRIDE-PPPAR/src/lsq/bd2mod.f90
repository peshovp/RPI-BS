...
!!
!
subroutine bd2mod(jd, sod, LCF, SITE, OB, SAT, IM)
  implicit none
...
! function called
  real*8        dot
  real*8        bds_rng_cor ! correct bds pseudorange obs bias
...
!! receiver clock correction
  if (index(LCF%isbsys,'C') .ne. 0) then
    do i=1, MAXSYS+1
      if(SITE%rclock(i).ne.0.d0) exit
    enddo
    SITE%rclock(4) = SITE%rclock(i)
  endif
  drecclk = 0.d0
  drecclk = SITE%rclock(4)/VLIGHT
!
...
!   BDS reconfiguration: only process BDS2 satellites (no satellites processed after reconfig)
    if (bds_system_class(prn_int, jd, sod) /= 2) cycle
...
!
!! correct bds pseudorange obs bias (only for BDS2)
    OB%obs(isat, 3) = OB%obs(isat, 3) + bds_rng_cor(OB%prn(isat), 1, OB%elev(isat)) ! in meters
    OB%obs(isat, 4) = OB%obs(isat, 4) + bds_rng_cor(OB%prn(isat), 3, OB%elev(isat)) ! in meters
!
...
!! observation equation
    call partial_gnss(OB%npar, dloudx, drate, OB%pname, OB%ltog(1, isat), rot_f2j, trpart, OB%amat(1, isat), 'C')
!
!! next satellite
  end do
!
!! receiver clock offset
  if (ite .lt. 2 .and. index(LCF%isbsys,'C').eq.0) then
    k = 0
    drecclk_tmp = 0.d0
    do isat = 1, LCF%nprn
      if (LCF%prn(isat)(1:1) .ne. 'C') cycle
      read (LCF%prn(isat)(2:3), '(i2)') prn_int
!     BDS reconfiguration: use only BDS2 satellites for clock estimation
      if (bds_system_class(prn_int, jd, sod) /= 2) cycle
    ...
    if (dabs(drecclk) .gt. 1.d-6 .or. (k .ge. 1 .and. SITE%rclock(4) .eq. 0.d0)) then
      SITE%rclock(4) = SITE%rclock(4) + drecclk*VLIGHT
      drecclk = SITE%rclock(4)/VLIGHT
      goto 100
