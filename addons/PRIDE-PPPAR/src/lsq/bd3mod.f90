...
!!
!
subroutine bd3mod(jd, sod, LCF, SITE, OB, SAT, IM)
  implicit none
...
!! receiver clock correction
  if (index(LCF%isbsys,'3') .ne. 0) then
    do i=1, MAXSYS+1
      if(SITE%rclock(i).ne.0.d0) exit
    enddo
    SITE%rclock(5) = SITE%rclock(i)
  endif
  drecclk = 0.d0
  drecclk = SITE%rclock(5)/VLIGHT
!
...
!! observation equation
    call partial_gnss(OB%npar, dloudx, drate, OB%pname, OB%ltog(1, isat), rot_f2j, trpart, OB%amat(1, isat), '3')
!
!! next satellite
  end do
!
!! receiver clock offset
  if (ite .lt. 2 .and. index(LCF%isbsys,'3').eq.0) then
    k = 0
    drecclk_tmp = 0.d0
    do isat = 1, LCF%nprn
      if (LCF%prn(isat)(1:1) .ne. 'C') cycle
...
!     BDS reconfiguration: use only BDS3 satellites for clock estimation
    ...
    if (dabs(drecclk) .gt. 1.d-6 .or. (k .ge. 1 .and. SITE%rclock(5) .eq. 0.d0)) then
      SITE%rclock(5) = SITE%rclock(5) + drecclk*VLIGHT
      drecclk = SITE%rclock(5)/VLIGHT
      goto 100
    !!  write (*, '(a,i7,f9.2,e15.4)') '***ERROR(bd3mod): abnormal drecclk at ', jd, sod, drecclk
    !!  call exit(1)
    end if
  end if
