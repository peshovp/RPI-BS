...
A live test on BaseStation with real Earthdata credentials subsequently found
...
def latest_ultra_rapid_hour_mark(dt: datetime) -> str:
    ...
    BaseStation, GPS week 2429/2430 range - e.g.
...
def generate_ultra_rapid_candidates(reference_dt: datetime, max_attempts: int = 8):
    ...
    BaseStation (GPS week 2430, captured 2026-08-04T15:55 UTC) showed the
    ...
    covers the ~1-day lag observed on BaseStation while remaining a small,
...
class PPPDownloader:
    ...
    def _ensure_session(self):
        ...
        BASESTATION WITH REAL EARTHDATA CREDENTIALS:
    ...
    def _build_url(self, product_type: str, tier: Tier, gps_date: GPSDate) -> str:
        ...
        directory listing on BaseStation
        ...
        authenticated CDDIS directory listing on BaseStation (GPS week
        2429/2430 range):
    ...
    def _fetch_ultra_rapid_with_retry(self, survey_start_time: datetime,
        ...
        is already confirmed working end-to-end on BaseStation and is left
...
#    TESTED on BaseStation with real Earthdata credentials and CONFIRMED
...
#    BaseStation to confirm it actually authenticates before Phase 2c/2d can
